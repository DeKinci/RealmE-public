...
Model *CubeForge::createCube(Position *position) {
    ...
    auto cube = new Model(*mesh, Shader::basicShader(), position);

    static Texture doggoTexture("doggo", GL_LINEAR, GL_TEXTURE0);
//    cube->shader.set("texture", 0);
    cube->setTexture(doggoTexture);

    return cube;
}

Model *CubeForge::createCube(int x, int y, int z) {
    return createCube(new Position((float) x, (float) y, (float) z, 1.f, 0.f, 0.f, 0.f));
}
